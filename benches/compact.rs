...
#[bench]
fn benchmark_compact_noop(b : &mut Bencher) {
    let test_string = black_box(r#"{
        "A longish bit of JSON": true,
        "containing": {
            "whitespace": " ",
    ...
    let mut test_vec : Vec<u8> = Vec::with_capacity(test_string.len());
    compact(test_string, &mut test_vec).unwrap();
    let mut output : Vec<u8> = Vec::with_capacity(test_vec.len());
    b.bytes = test_vec.len() as u64;
    b.iter(|| { output.clear(); compact(&mut test_vec, &mut output) });
}
